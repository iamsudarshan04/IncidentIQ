...
const RegisterPage = () => {
  ...
  // Default to light mode for this specific page to match screenshot closely
  useEffect(() => {
    if (isDark) {
    ...
    <div className={`min-h-screen w-full transition-colors duration-500 flex ${isDark ? 'auth-grid-dark text-[#FAFAFA]' : 'auth-grid-light text-slate-900'} relative overflow-hidden`}>
      ...
      <div className="max-w-[1400px] mx-auto w-full flex flex-col lg:flex-row relative z-10 px-6 lg:px-12 py-12 min-h-screen">
        ...
        <div className="w-full lg:w-[500px] flex items-center justify-center">
          ...
          >
            <div className="mb-8">
              <h2 className={`text-3xl font-bold tracking-tight mb-2 ${isDark ? 'text-white' : 'text-slate-900'}`}>Create account</h2>
              <p className={isDark ? 'text-[#A1A1AA]' : 'text-slate-500'}>Sign up to access your intelligence dashboard.</p>
            </div>

            {/* Segmented Control / Tabs */}
            <div className={`flex relative rounded-xl p-1 mb-8 ${isDark ? 'bg-[#09090B] border border-[#27272A]' : 'bg-slate-100'}`}>
              <div className="absolute right-1 top-1 bottom-1 w-[calc(50%-4px)] bg-white dark:bg-[#18181B] rounded-lg shadow-sm border border-slate-200 dark:border-[#3F3F46] z-0 transition-transform duration-300 ease-in-out" />
              <Link to="/login" className={`flex-1 relative z-10 py-2.5 text-sm font-medium text-center rounded-lg transition-colors ${isDark ? 'text-[#A1A1AA] hover:text-white' : 'text-slate-500 hover:text-slate-900'}`}>
                Sign in
              </Link>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <button 
                type="submit" 
                disabled={loading}
                className={`w-full mt-4 font-bold rounded-xl px-4 py-3.5 flex items-center justify-center gap-2 transition-all disabled:opacity-70 disabled:cursor-not-allowed group ${isDark ? 'bg-gradient-to-r from-[#A855F7] to-[#8B5CF6] text-white hover:brightness-110 shadow-lg shadow-[#A855F7]/25' : 'bg-blue-600 text-white hover:bg-blue-700 shadow-lg shadow-blue-600/20'}`}
              >
